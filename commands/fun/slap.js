...
module.exports = {
    ...
    run: async (client, message, args) => {
        var slaprand = Math.floor(Math.random() * (slapimgs.length));
      
      var genslapimg = slapimgs[slaprand];
      ...
      const slapembedno = new Discord.MessageEmbed()
          .setDescription(`**${message.author.username}**, slapped someone that he don't want us to know!`)
          .setImage(`${genslapimg}`)
          
      
      const slapembedyes = new Discord.MessageEmbed()
          .setDescription(`**${message.author.username}**, slapped the DAMN out of ${slapusr}`)
          .setImage(`${genslapimg}`)
          .setColor(`#fffff1`)
